 ...
 *
 * This one took three attempts to find, and the reason is worth keeping.
 *
 * statutes.capitol.texas.gov returns HTTP 200 with an IDENTICAL 250,874-byte
 * Angular shell for EVERY path, including /this/does/not/exist.htm. No amount
 * of probing that host can produce a document or a miss, because the statutes
 * are not served from it at all. The real location is in an environment
 * constant inside chunk-7GRZWKYH.js — a chunk that index.html never
 * references, so a sweep of preloaded chunks misses it:
 *
 *   TCASCore:       https://tcss.legis.texas.gov/api/
 *   FileServerPath: https://tcss.legis.texas.gov/resources
 *
 * The API needs numeric code ids that are not derivable from a citation, but
 * the FILE SERVER does not: it holds the same per-chapter HTML the old site
 * used, at the same path minus the Docs/ prefix.
 *
 *   /resources/PE/htm/PE.19.htm   -> Penal Code chapter 19, 15,492 bytes
 *   /resources/PE/htm/PE.9999.htm -> honest 404
 *
 * FILES ARE PER CHAPTER, CITATIONS ARE PER SECTION. "Penal Code 19.02" lives
 * inside PE.19.htm alongside 19.01 through 19.06, so the chapter is fetched
 * and the section cut out of it — bounded by the next "Sec. N.NN." heading.
 * Returning the whole chapter would hand back six statutes when one was asked
 * for.
 */
...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'Texas Statutes');
}

const FILES = 'https://tcss.legis.texas.gov/resources';
const UA = 'pipeworx-mcp-texas-code/1.0 (+https://pipeworx.io)';

const CODES: Record<string, string> = {
  AG: 'Agriculture', AL: 'Alcoholic Beverage', BC: 'Business and Commerce', BO: 'Business Organizations',
  CP: 'Civil Practice and Remedies', CR: 'Code of Criminal Procedure', ED: 'Education', EL: 'Election',
  ES: 'Estates', FA: 'Family', FI: 'Finance', GV: 'Government', HR: 'Human Resources',
  HS: 'Health and Safety', IN: 'Insurance', LA: 'Labor', NR: 'Natural Resources', OC: 'Occupations',
  PE: 'Penal', PR: 'Property', PW: 'Parks and Wildlife', SD: 'Special District Local Laws',
  TX: 'Tax', TN: 'Transportation', UT: 'Utilities', WA: 'Water',
};
...
const tools: McpToolExport['tools'] = [
  {
    ...
    inputSchema: {
      type: 'object',
      properties: {
        code: { type: 'string', description: 'Texas code abbreviation, e.g. "PE" (Penal), "FA" (Family), "CP" (Civil Practice and Remedies), "TX" (Tax). Call tx_codes to see them.' },
        section: { type: 'string', description: 'Section as cited, e.g. "19.02" or "6.001". The chapter is the part before the dot.' },
      },
...
async function callTool(name: string, args: Record<string, unknown>): Promise<unknown> {
  if (name === 'tx_codes') {
    return {
      returned: Object.keys(CODES).length,
      codes: Object.entries(CODES).map(([code, subject]) => ({ code, name: `${subject} Code` })),
      source: 'Texas Constitution and Statutes',
    };
  }
  if (name !== 'tx_statute') throw new Error(`Unknown tool: ${name}`);

  const code = String(args.code ?? '').trim().toUpperCase().replace(/[^A-Z]/g, '');
  const raw = String(args.section ?? '').trim().replace(/\s+/g, '');
  if (!CODES[code]) {
    return { found: false, reason: 'invalid_code',
      message: `"${args.code}" is not a Texas code abbreviation.`,
      hint: 'Call tx_codes to see them. Crimes is PE, family law is FA, civil suits are CP.' };
  }
  const m = /^(\d+[A-Za-z]?)\.(\d+[A-Za-z]?)$/.exec(raw);
  if (!m) {
    return { found: false, reason: 'unparsed_section',
      message: `"${args.section}" is not a Texas section citation.`,
      hint: 'Texas sections are chapter.section, e.g. "19.02" (murder) or "6.001" (divorce).' };
  }
  const chapter = m[1];

  const url = `${FILES}/${code}/htm/${code}.${chapter}.htm`;
  const res = await pwFetch(url, { headers: { 'User-Agent': UA } });
  if (res.status === 404) {
    return { found: false, reason: 'chapter_not_found', code, chapter, section: raw,
      message: `No chapter ${chapter} in the Texas ${CODES[code]} Code (HTTP 404).`,
      hint: 'Chapter numbers are not contiguous — repealed chapters are normal.' };
  }
  if (!res.ok) throw new Error(`Texas file server returned HTTP ${res.status} for ${code}.${chapter}`);

  const text = statuteText(await res.text());

  // Cut the requested section out of the chapter. The heading is
  // "Sec. 19.02.  MURDER." and the section ends where the next "Sec. N.NN."
  // begins — returning the chapter would hand back six statutes for one ask.
  const esc = raw.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
  const start = text.search(new RegExp(`Sec\\.\\s*${esc}\\.`));
  if (start === -1) {
    return { found: false, reason: 'section_not_found', code, chapter, section: raw,
      message: `Chapter ${chapter} of the Texas ${CODES[code]} Code exists but has no section ${raw}.`,
      hint: 'Section numbers are not contiguous — repealed sections are normal.' };
  }
  const rest = text.slice(start + 4);
  const nextRel = rest.search(/Sec\.\s*\d+[A-Za-z]?\.\d+[A-Za-z]?\./);
  const body = (nextRel === -1 ? text.slice(start) : text.slice(start, start + 4 + nextRel)).trim();

  const heading = (body.match(/^Sec\.\s*[\d.A-Za-z]+\.\s*([^.]{2,120})\./) ?? [])[1] ?? null;
  const history = (body.match(/\n((?:Acts|Added by|Amended by)[\s\S]{0,600})$/) ?? [])[1] ?? null;

  return {
    citation: `Tex. ${CODES[code]} Code § ${raw}`,
    code, code_name: `${CODES[code]} Code`, chapter, section: raw,
    heading: heading ? heading.trim() : null,
    text: body, text_chars: body.length,
    history: history ? history.trim().slice(0, 600) : null,
    url,
    source: 'Texas Legislative Council file server (tcss.legis.texas.gov), keyless',
    attribution: 'Texas statutes are public record.',
  };
}
